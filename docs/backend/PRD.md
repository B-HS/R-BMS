...
> 대상: rbms 전용 IR-슈퍼셋 백엔드. **역사 PRD + 구현 기준:** 핵심 요구는 `web/`에 구현됐습니다. 현재 계약·제약은 [api-spec.md](./api-spec.md)와 [endpoint-tasks.md](./endpoint-tasks.md), 실제 구현은 `web/src/`가 정본입니다. Phase R의 실제 키·배포 검증은 보류입니다.

## 1. 배경 / 문제
- rbms는 오프라인으로 완전 동작(로컬 `scores.ron`·`replays/`·`settings.ron`). 그러나 **기기 간 동기화·랭킹·라이벌·발광 표 연동**이 없다.
- BMS 생태계의 표준은 **IR(Internet Ranking)**: 원조 **LR2IR**(md5 기반), 현대 **레퍼런스 구현 IR**(sha256 기반). rbms는 `rbms-ir` 클라이언트와 `web/`의 IR-슈퍼셋 서버를 갖췄습니다.
- 요구의 구현 상태: 점수·랭킹·인증·설정 동기화·리플레이·코스·표·웹 조회는 구현됐고, LR2IR XML 어댑터와 리플레이 재시뮬레이션 검증은 후속입니다.

## 2. 목표 (Goals)
1. **스코어 IR**: 차트(md5·sha256)별 랭킹 제출/조회, 개인 베스트, 라이벌. 레퍼런스 구현 IRScoreData의 풀 데이터(판정 early/late, 옵션, 시드, 게이지, 룰) 보존.
...
6. **인증**: 계정 register/login(레퍼런스 구현 IRAccount 슈퍼셋) → 토큰(네이티브) + 세션 쿠키(웹 FE). 익명 `guest` 옵션.
...
9. **웹 FE**: 구현된 검색/탐색/리더보드/리플레이 뷰어 `/api/fe/*` 경로 + envelope·세션. Next 단일 오리진이므로 CORS는 현재 범위가 아니다.
...
- **레퍼런스 구현 사용자**: (호환 모드) 레퍼런스 구현 클라이언트가 본 서버를 IR로 등록해 sha256 스코어 제출/조회.
...
## 6. 비기능 요구 (NFR)
- **호환**: 단일 클라이언트가 md5만(LR2 차트) / sha256만(레퍼런스 구현) 보내도 동작. 미지원 기능은 `capabilities`로 광고 → 클라가 UI 토글.
...
- 레퍼런스 구현 클라이언트가 (호환 모드) sha256 스코어 제출/조회 성공.
...
- **라이선스**: `web/`은 현재 이 저장소 안에 있으며 별도 MIT 레포 전제는 폐기됐습니다. 별도 배포·라이선스 분리는 향후 결정 사항입니다.
