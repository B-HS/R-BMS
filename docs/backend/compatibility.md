...
> LR2IR(원조) · 레퍼런스 구현 IR(현대) ↔ rbms 슈퍼셋 필드 대응. 모든 매핑은 **원본 소스/실프로토콜 대조**(아래 출처).

## 1. 차트 키
| | 키 | 길이 |
|---|---|---|
| LR2IR | `bmsmd5` (MD5) | 32 hex |
| reference IR | `sha256` | 64 hex |
| **rbms** | **md5 + sha256 동시 보유** | `{hash}` 경로는 길이로 판별 |
- LR2 코스/차트 구분: `len(hash)==32 → bms, else course`(lr2irscraper `bmsmd5.py`). 코스 해시 = 구성 차트 해시 결합.
- rbms `course_hash`: 코스 차트 **sha256를 순서대로 concat → SHA-256**(레퍼런스 구현 관행 준용). LR2 어댑터는 md5 결합 변형도 허용.

## 2. 클리어 램프
| id | the reference implementation `ClearType` | LR2IR `clear`(1-5) | rbms `ClearLamp` |
|---|---|---|---|
...
- 출처: 레퍼런스 구현 `ClearType.java`(id 0-10), lr2irscraper `ranking.py`("clear は 1-5 … FAILED, EASY, CLEAR, HARD, FULLCOMBO").

## 3. 판정 / 스코어
| 항목 | LR2IR XML | 레퍼런스 구현 IRScoreData | rbms |
|---|---|---|---|
...
- 출처: 레퍼런스 구현 `IRScoreData.java`(early/late 12필드 + `getExscore()=(epg+lpg)*2+egr+lgr`), lr2irscraper `ranking.py`(XML 컬럼 `name,id,clear,notes,combo,pg,gr,minbp`).
...
- 출처: 레퍼런스 구현 `IRScoreData.java`·`JudgeAlgorithm.java`(Combo/Duration/Lowest/Score).

## 5. 인증
| | 방식 |
|---|---|
| LR2IR | per-user 제출(클라가 user 세션 보유, `gateway.cgi` 난독) |
| the reference implementation | `IRAccount{id,password,name}` → `register`/`login` → `IRPlayerData{id,name,rank}` |
| **rbms** | register/login(IRAccount 슈퍼셋, email 추가) → **Bearer 토큰**, guest 허용 |
- 출처: 레퍼런스 구현 `IRConnection.java`(register/login), `IRAccount.java`, `IRPlayerData.java`.

## 6. 레퍼런스 구현 IRConnection ↔ rbms 엔드포인트
| 레퍼런스 구현 메서드 | rbms 엔드포인트 |
|---|---|
...
- 출처: 레퍼런스 구현 `IRConnection.java`(인터페이스 전체).
...
- 레퍼런스 구현 IR 생태계: [exch-bms2/레퍼런스 구현 Wiki — IR Tips](https://github.com/exch-bms2/<reference>/wiki/IR-Tips), [Mocha-Repository](https://mocha-repository.info/), [GAFTALK 레퍼런스 구현 IR](https://www.gaftalk.com/blog/items/레퍼런스 구현-ir/).
