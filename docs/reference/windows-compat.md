...
> **상태(2026-09-16):** 자동 빌드·테스트와 무인자 진입은 완료했습니다. Windows GUI·WASAPI·실장치 입력·CJK 폰트는 아직 수동 QA가 필요하며, 릴리스 서명은 Phase R에서 사용자 키와 함께 수행합니다. 현재 정본은 [roadmap](../roadmap.md)입니다.
...
## CI가 보장하는 것
- `.github/workflows/ci.yml`이 **ubuntu·macos·windows 매트릭스**로 `cargo build --workspace` + `cargo test --workspace`를 수행합니다. 괄호의 887은 2026-06-03 당시 수치이며, 현재 기준은 2026-09-16 Phase H `cargo test --workspace` exit 0, 등록 테스트 3,040개입니다. Windows에서 컴파일·단위테스트(로직)는 green입니다.
...
- [x] **무인자 실행** — 더블클릭(인자 없음)도 GUI 곡선택으로 진입한다.

## 알려진 크로스플랫폼 한계
- **창 리사이즈 UI 리플로우 없음** — 논리 1280×720 고정(Windows/macOS 공통). → Phase 7 cosmetic.
- **미서명 exe → SmartScreen 경고**(하드 차단 아님). 서명은 Phase R에서 사용자 키로 수행한다. → `docs/ci-release.md`.
