# 레퍼런스 구현 비판정 기능 패리티 보고서 — 적대적 검증

대상 보고서: `scratchpad/research/feature-inventory.md` (findings 01~19)
검증 방식: 인용된 rbms/레퍼런스 구현 파일을 직접 통독. 수치(정렬 종류·enum 개수 등)는 원본에서 재계수.
...
| 16 | partially | 사실. 단 레퍼런스 구현 KeyCommand는 11종이 아니라 **13종** |
| 17 | confirmed | Stage에 Decide 없음 |
| 18 | uncertain | rbms 측 부재는 확인. 레퍼런스 구현 `external/`·`stream/` 디렉토리는 시간상 미확인 |
...
"2중 해시 매번" 도 사실이다. 레퍼런스 구현 측 `song/SQLiteSongDatabaseAccessor.java:461 updateSongDatas(..., updateAll, ...)`,
...
레퍼런스 구현 측 `input/` 디렉토리에 `BMControllerInputProcessor.java`, `MidiInputProcessor.java`,
...
레퍼런스 구현 측 `CourseData.java`·`CourseDataAccessor.java`(루트), `result/CourseResult.java`·`CourseResultSkin.java` 실재 확인.

### 06 필터·정렬·즐겨찾기 — partially

정렬 수치는 양쪽 다 정확하다. rbms `main.rs:543-566` `SortMode` 는 Default/Title/Artist/Level/Clear 5종,
레퍼런스 구현 `select/BarSorter.java` enum 상수를 재계수하면 TITLE(:19)·ARTIST(:47)·BPM(:65)·LENGTH(:83)·LEVEL(:101)·
...
레퍼런스 구현 측 `select/DifficultyFilter.java`·`ModeFilter.java` 실재는 `ls select/` 로 확인.
...
the reference implementation `AudioConfig.java:46 systemvolume`, `:50 keyvolume`, `:54 bgvolume`, `:24 deviceBufferSize`,
...
레퍼런스 구현 `select/MusicSelector.java:76-77` 의 `rankingDuration = 5000`, `rankingReloadDuration = 10*60*1000`,
...
레퍼런스 구현 `play/PracticeConfiguration.java`·`play/SkinPractice.java`·`pattern/PracticeModifier.java` 실재 확인.
...
(b) 레퍼런스 구현 `PlayerConfig.java:169 autosavereplay[]` 같은 클리어타입별 슬롯 및 `select/MusicSelectCommand.java` 의
...
레퍼런스 구현 `pattern/` 디렉토리 `ls` 결과 `ExtraNoteModifier`·`MineNoteModifier`·`LongNoteModifier`·
...
레퍼런스 구현 `result/SkinGaugeGraphObject.java`·`play/TargetProperty.java` 실재, `PlayerConfig.java:72 targetid = "MAX"` 라인 일치.
...
**정정**: 레퍼런스 구현 `input/KeyCommand.java:5-17` 을 직접 세면 SHOW_FPS, UPDATE_FOLDER, OPEN_EXPLORER,
...
다만 레퍼런스 구현 `external/`·`stream/` 디렉토리와 `PlayerConfig.java:247-258` 은 **시간 제약으로 직접 열지 못했다**.
...
§0(:83)이 "완전 플레이 가능 + 레퍼런스 구현/IIDX 확장 다수 완성" 이라 기술한다.
...
| # | 항목 | 근거 |
|---|---|---|
| M1 | **스킨 시스템 패리티** — 레퍼런스 구현는 LR2/JSON/Lua 3종 스킨 로더 + 스킨 설정 화면(OPEN_SKIN_CONFIGURATION)을 가지지만 rbms는 자체 RON 테마 1종 | 레퍼런스 구현 `skin/SkinLoader.java`, `skin/{json,lr2,lua}/`(디렉토리 실재), `SkinConfig.java`(루트) / rbms `crates/rbms-render/src/{skin.rs,theme.rs}` 만 존재, PROCESS.md:177 "남은 건 F5 결과/메뉴 패널 위치 RON화" |
| M2 | **시스템 사운드 전무** — 곡선택 커서/결정/클리어 효과음 계층이 없다 | 레퍼런스 구현 `SystemSoundManager.java`(루트) / rbms `grep -i "SystemSound"` 히트 0건 |
| M3 | **라이벌 기능 미배선** — IR 트레잇에 `rivals`/`player_profile` 이 이미 있는데 클라이언트에서 호출·표시하지 않는다(정렬 RIVALCOMPARE_* 2종의 전제) | rbms `crates/rbms-ir/src/http.rs:85,89-90` 정의 존재 / `apps/` 호출부 0건, 레퍼런스 구현 `RivalDataAccessor.java`(루트) |
...
| M6 | **플레이어 프로필/통계 계층 부재** — 레퍼런스 구현는 PlayerData/PlayerInformation/PlayDataAccessor로 총 플레이수·램프 분포·표별 달성률을 관리하지만 rbms는 평면 `Vec<ScoreRecord>` 뿐 | 레퍼런스 구현 `PlayerData.java`·`PlayerInformation.java`·`PlayDataAccessor.java`(루트) / rbms `apps/rbms-player/src/scores.rs:30-32` |
| M7 | **런처(설정 GUI) 계층 부재** — 레퍼런스 구현는 JavaFX 런처로 오디오/입력/IR/폴더/코스 에디터를 제공(코스 에디터 포함) | 레퍼런스 구현 `launcher/{AudioConfigurationView,InputConfigurationView,IRConfigurationView,CourseEditorView}.java` / rbms는 인게임 SETTING_TABS(`main.rs:598~`)만 |
| M8 | **랜덤 코스/랜덤 스테이지 미구현** — finding-05가 고정 코스만 다루고 랜덤 코스는 언급 없음 | 레퍼런스 구현 `select/RandomCourseData.java`·`select/RandomStageData.java` / rbms Course 심볼 0건 |

## 미조사 범위

- 레퍼런스 구현 `external/`·`stream/` 디렉토리 내용(finding-18의 절반) — 파일 미열람.
- 레퍼런스 구현 `play/BMSPlayer.java` 본문(finding-10의 일시정지 처리 실제 구현) — 보고서도 [미확인]으로 표기, 본 검증에서도 미열람.
