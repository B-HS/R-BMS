...
- [확정] 見逃し POOR → `matcher.rs:306-311` `events.push(Judge::Miss)` → `gauge.update(Miss)` = deltas[5](MS: NORMAL -2, HARD -5). beatoraja `JudgeManager.java:595` `updateMicro(...,4,...)` → value[4](POOR: NORMAL -6, HARD -10). 空POOR 는 `matcher.rs:231-235` Miss(-2) 로 beatoraja MS 와 일치. → 자동미스 게이지 페널티가 절반~1/3. **critical, S 효력**(sweep 1줄 + 카운트 라벨 + 테스트). PROCESS.md §6 "게이지 대조 검증됨" 은 이 점에서 오류.
- [확정] `windows.rs`: 5K → SEVENKEY_NOTE(`note_for_mode` `_ =>`), POPN 복제(주석에 pending 명시), SEVENKEY_LN_END=(120,150,200,±250)+MS 는 beatoraja FIVEKEYS LN end 값(SEVENKEYS 는 120,160,200,(-280,220), MS 없음). `LN_MARGIN=200_000` 전모드(beatoraja 7K/5K/KB=0, PMS=200000). `scaled()` 가 bd 까지 스케일(beatoraja judgeWindowRate 는 i<3 만 + 클램프). 스크래치 전용 윈도우 없음.
...
- [부분] FLOATING HI-SPEED: rbms `constant_speed` = 그린넘버 고정(2000/hispeed) + 곡 내 BPM 변화 무시(beatoraja CONSTANT). IIDX FLOATING/beatoraja MAIN/MAX/MIN/START BPM 기준 hispeed 자동조정(곡 내 BPM 변화는 반영)은 없음. 라벨 "FLOATING" 이 IIDX 의미와 반대(rbms FLOATING = 일반 hi-speed).
...
## skin-system.md (research:skin-beatoraja)
...
- → 결정 필요: (a) 자체 RON 오브젝트 스킨(beatoraja 모델 미러, Lua 없음) (b) JSON 호환 + 소형 Lua 식 평가기(mlua) (c) LR2 CSV 까지. 규모 5.6~8.1k LOC(+LR2 1.5~2.5k).
- 프로퍼티 968개 중 play 100~150, select/result 포함 250~350.

## judge-gauge.md (research:judge-parity)
- 위 직접 검증 항목 외: JudgeAlgorithm Duration 고정(beatoraja 기본 Combo), 지뢰 데미지 없음, MODIFY_DAMAGE 없음, CLASS 게이지 없음, 5K/PMS/KB/LR2 게이지 없음, 9게이지 병렬 갱신 없음(GAS 불가), lnmode 강제 없음, #DEFEXRANK 미파싱, TOTAL fallback 200 vs validate 공식 max(260, 7.605n/(0.01n+6.5)), CN deferral 없음, LN 미릴리스 확정 규칙 다름(beatoraja 7K 는 마진 0 + 머리판정 확정), fast/slow dm==0 반대, 판정된 노트 재타격 空POOR 없음.
...
## feature-inventory.md (research:beatoraja-parity)
...
## 검증 보고서 반영 (verify)
- iidx-verify: 14건 중 confirmed 11·partially 3(04 시작전패널 severity high→medium, 07 LEGACY NOTE 는 S, 11 게이지 폴백은 GAS 포팅으로 교체). missed: M1 그린넘버 LIFT 미반영(Fable 확정), M2 assist 항상 빈 배열(Fable 확정), M3 GAS, M4 레인커버 5% 계단, M5 hispeed 0.25 step/0.5~10(beatoraja 0.01~20), M6 HUD 그래프도 3밴드, M7 2000.0 매직상수 2곳, M8 ARENA 축 없음.
