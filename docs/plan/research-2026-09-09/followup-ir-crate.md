...
| LR2IR/레퍼런스 구현 관계 | 직접 호환 아님. `docs/backend/compatibility.md:1-119` 가 **매핑표**로만 존재(§2 램프 다운매핑, §3 early/late 12필드, §6 `IRConnection` 메서드↔rbms 엔드포인트, §7 LR2IR `getrankingxml.cgi`는 "어댑터 대상", `gateway.cgi` 제출은 "재현 비범위") |

DTO 슈퍼셋 요소: `ChartId{md5,sha256}`(`dto.rs:7-11`), `JudgeBreakdown`의 레퍼런스 구현 12필드 `epg..lms`+`avgjudge`+rbms 고유 `empty_poor`(`dto.rs:64-92`), `PlayOptions`의 `option`(레퍼런스 구현 원시 비트마스크)·`seed`·`judge_algorithm`·`rule`·`skin`·`client_build_sha256`(`dto.rs:104-129,150-170`), 모든 DTO의 `extra: HashMap<String,Value>`, `ServerCapabilities`(`dto.rs:196-204`).
...
| 차트 키 | URL 은 **md5**(DTO 는 md5+sha256 동시 보유) — 레퍼런스 구현 는 sha256 키 | `http.rs:76,80,101`; `레퍼런스 구현 IRScoreData.java:19`, `RankingDataCache.java:41` |
...
## Q2. 레퍼런스 구현 `ir/` 아키텍처 대비 격차

| 축 | 레퍼런스 구현 | rbms 현재 | 격차 |
|---|---|---|---|
...
| 복수 IR | `IRStatus[]` 배열(`MainController.java:92,166-182`) — 플레이어 설정의 `IRConfig` 마다 로그인 시도. **단 랭킹 조회는 `ir[0]` 고정**(`RankingData.java:73-79`) | 단일 `Arc<dyn ScoreServer>`(`main.rs:169-176`) | 중간. 실사용상 레퍼런스 구현 도 사실상 1개 |
...
| # | 변경 | 위치(신규/수정) | 내용 | 효력 |
|---|---|---|---|---|
| 1 | 랭킹 캐시 + 상태 | 신규 `apps/rbms-player/src/ir_ranking.rs` | `RankingState{Idle,Loading,Ready,Failed}` + `HashMap<(String sha256, u8 lnmode), RankingEntry>`. `RankingEntry{rows:Vec<ScoreRecord>, my_rank, local_rank, prev_rank, lamps:[u32;11], fetched_at}` — 레퍼런스 구현 `RankingData.java:96-140` 후처리 이식 | **M** |
...
| 3 | 포커스 디바운스 트리거 | `apps/rbms-player/src/app_select.rs:296-305` | 프리뷰의 `PREVIEW_DEBOUNCE_FRAMES` 디바운스를 그대로 재사용해 커서 정지 후 1회 조회(= 레퍼런스 구현 `currentRankingDuration`) | **S** |
| 4 | 캐시 키 결정 | `crates/rbms-ir/src/http.rs:76,80` | 현재 URL 키가 md5. <reference>/스펙은 sha256 우선(`compatibility.md:5-11` "길이로 판별"). **캐시 키는 sha256+lnmode 로 고정**하고 URL 키는 서버 계약 확정 시 일치시킴 | **S**(캐시) / **M**(URL 전환 시 계약 변경) |
...
| 8 | target 소스 배선 | `apps/rbms-player/src/app_play.rs:232` | 플레이 진입 시 캐시 엔트리에서 목표 EX 산출(`NEXT n / RANK n / TOP n%` — 레퍼런스 구현 `TargetProperty.java:417-430` 이식). 캐시 미스면 target 없이 진행(폴링 스레드 만들지 않음 — 레퍼런스 구현 의 500ms 폴링은 이식하지 않는 편이 단순) | **M** |
