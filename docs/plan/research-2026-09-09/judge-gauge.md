# rbms-judge vs 레퍼런스 구현 판정/게이지 함수 단위 대조
...
- the reference implementation: `<reference>/play/{JudgeProperty,JudgeManager,JudgeAlgorithm,BMSPlayerRule,GrooveGauge,GaugeProperty}.java`, `PlayerConfig.java`, `PlayConfig.java`

---

## 0. 인덱스 규약 (가장 중요한 전제)

레퍼런스 구현의 판정 인덱스는 두 계열이 있다.
...
rbms `Judge` enum(`crates/rbms-judge/src/lib.rs:12-19`)은 `PerfectGreat, Great, Good, Bad, Poor, Miss` → 인덱스 4=Poor, 5=Miss. 즉 **rbms의 `Judge::Miss`는 레퍼런스 구현의 MS(空POOR) 슬롯**이고, 레퍼런스 구현의 見逃しPOOR(코드 4)는 rbms의 `Judge::Poor` 슬롯에 대응한다. 이 매핑이 뒤집혀 있는 곳이 아래 D-01이다.
...
| 見逃しPOOR 판정 코드 | **4 (POOR)** → 게이지 `value[4]` (`JudgeManager.java:595`) | `Judge::Miss` = 인덱스 5 → 게이지 `deltas[5]`(= 레퍼런스 구현 MS 값) (`matcher.rs:311`) | **divergent (D-01, 최중요)** |
...
| LN 미릴리스 | `processing.getMicroTime() < mtime`이면 `lnstartJudge`로 확정 (`JudgeManager.java:575-580`) | `now > end + LN_MARGIN(200ms)`일 때 `ln_end.judge(end-now)`로 확정, LN은 worse(head,end) (`matcher.rs:288-297`) | **divergent** — 레퍼런스 구현 7K는 마진 0이고 **머리 판정으로 확정**, rbms는 200ms 뒤 종단 윈도우 판정 |
...
| 게이지 | 레퍼런스 구현 (modifier, min, max, init, border, value[6], guts) | rbms | 판정 |
|---|---|---|---|
...
| 항목 | 레퍼런스 구현 | rbms | 판정 |
|---|---|---|---|
| TOTAL modifier | `f > 0 ? f * total / totalNotes : f` (`GrooveGauge.java:262`) | 동일, 단 `total<=0`이면 **200.0** fallback (`gauge.rs:77-86`) | **divergent** — 레퍼런스 구현는 `BMSPlayerRule.validate`가 미리 `max(260, 7.605n/(0.01n+6.5))`로 채움 (`BMSPlayerRule.java:88-93`). rbms는 `meta.total`에 그 공식을 적용하지 않고(`crates/rbms-chart/src/lib.rs:196`은 헤더 값 or 0.0) 200을 씀. 같은 공식이 rbms-chart:406에 있으나 노트밀도 계산 전용 |
...
레퍼런스 구현 `PlayerConfig`/`PlayConfig` 중 판정·게이지에 영향 있는 항목:

| 레퍼런스 구현 설정 | 위치 | rbms 대응 | 상태 / 효력 |
|---|---|---|---|
...
| — (rbms 고유) | — | `total_override` (`settings.rs:23`, UI 13번) | 레퍼런스 구현에 없는 rbms 확장 |
...
| 문서 | 상태 |
|---|---|
| `docs/reference/cn-hcn-judgment.md` | 본문 내용은 코드와 **일치**(2-판정 모델, `is_charge` 게이트, HCN 연속 게이지·CN deferral·BSS 잔여 명시). 단 원본 경로가 `/Users/hyunseokbyun/<reference>/...`로 적혀 있어 실제 경로(`<reference>/`)와 다름 — 경로 stale |
| `docs/acknowledge/reference-divergences.md` | CN/HCN 항목(27-32행)은 코드와 일치. 그러나 §1~§3에서 확인된 **판정 윈도우(5K/PMS/스크래치/LN-end)·게이지 세트(5K/PMS/KB/LR2/CLASS)·見逃しPOOR 게이지 인덱스·MODIFY_DAMAGE·지뢰 데미지·JudgeAlgorithm** 발산이 전혀 기록돼 있지 않음 → **불완전(누락형 stale)** |
| `docs/reference/mechanics.md` | 시간 상한으로 통독 못 함 — **미조사** |
| `docs/PROCESS.md` §7 한계 | 시간 상한으로 통독 못 함 — **미조사** |
| `windows.rs:22` 독스트링 "Values are 레퍼런스 구현 JudgeProperty 7K NOTE table" | NOTE는 사실. 그러나 `SEVENKEY_LN_END`(`:29` "레퍼런스 구현 JudgeProperty longnote end, 7K")는 실제로는 **FIVEKEYS longnote 값** → 주석 stale |
...
- `ClearType.java` 열거 전문 및 rbms `clear_lamp`의 FC/Perfect/Max 조건이 레퍼런스 구현과 같은지 미대조.
