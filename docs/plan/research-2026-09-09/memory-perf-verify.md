...
검증 범위는 rbms 측 코드 전수 확인이며, beatoraja 대조는 원 보고서가 전 항목 "미조사"로 남긴 부분이라 이번에도 열지 않았다(미조사 범위).
...
- beatoraja Java 측 대응 구조(`select/`, `audio/`) 대조 — 원 보고서도 전 항목 미조사였고 이번 검증에서도 열지 않았다. "beatoraja는 이렇게 한다"는 주장은 양쪽 어디에도 근거가 없다.
