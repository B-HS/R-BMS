...
상태: L1 연구 반영(2026-09-18). 결정은 `docs/acknowledge/2026-09-18-skin-followups-decisions.md` E1~E10. 사용자 지시(2026-09-18): "커밋 푸시하고, 요약한 내용의 남은 기능들도 다 구현 시작해서 커밋·푸시·머지". 남은 기능은 `docs/HANDOFF.md` §8 TODO 3(기본 번들 미세 결함)과 TODO 4(사양 §12 후속)이다. 실제 GPU 창 확인(TODO 2)은 터미널 화면 녹화 권한이 필요해 이 작업에 넣지 않는다. 브랜치 `feat/skin-followups`에서 작업하고 단위별 커밋 뒤 `dev`에 머지·푸시한다(`prod`는 Phase R, 키 필요).

연구(L1)로 확정한 사실은 결정 문서에 적었고, 남은 불확실성은 각 갈래 보고에서 다룬다. 레퍼런스 구현과 외부 묶음의 이름은 어디에도 적지 않는다.
...
| F5 24키 | 문서 타입 16(레퍼런스 PLAY_24KEYS, 이 빌드 표와 일치), `Mode::ALL` 편입, 26레인 기본 키, `play-24k.json5` 실제 문서, bmson `mode_hint` 경로 캡처 | 24키 bmson 차트로 헤드리스 캡처 1장, `is_supported_skin_type` 이 24키를 포함 |
...
| F7 pmchara | 팝픈 캐릭터 객체(`src`·`color`·`type`·`side`): `.chp` 정의(MS932) 로더, 타입 0~15, 모션 타이머 900~908, 좌상단 기준 축별 배율, 기본 번들 생성 캐릭터 | 픽스처 시트로 상태(대기·굿·배드·피버·승·패) 전환 픽셀 테스트 |
| F8 비디오 BGA | 비디오 파일 BGA 재생(`#BMPxx` 가 동영상일 때) | 새 크레이트 `rbms-video`: `.mpg`(mpeg-ps + mpeg-pes + oxideav-mpeg12video)·`.mp4`(re_mp4 + rusty_h264-decoder) feature 백엔드 중 stable 컴파일·ffmpeg 픽스처 대조를 통과한 것만 활성. 레퍼런스 동기 모델(전진 디코드·오프셋·프레임 드롭·EOF 공백). `.wmv` 미지원 경고 후 정지 이미지 폴백 |
...
- E5 24키 문서 타입 id 16 은 레퍼런스 PLAY_24KEYS 와 일치함을 확인했다. 17(DOUBLE)·18(BATTLE) 은 모드가 없어 제외.
