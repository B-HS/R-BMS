...
- **Public API**:
  ...
  - `render_result` + `ResultView`; `dj_rank`, `draw_rank_bar`, `ex_delta_label`, `RANK_BANDS`. `ResultExtras` (target, run-again hint) and `ResultContent` mark which native result groups a skin document replaces.
  - `content.rs`: `PlayContent` / `SelectContent` / `ScreenContent` — the per-screen flags a document's `replace` list turns on; `render_hud_with_content` and `render_select_on_background_with_content` skip the native groups the document owns (`hud.rs`, `select.rs`).
  - `skin_render` (JSON skin documents): `screen` (compile + draw a `LoadedSkin`), `state` (`PlayViewState` / `SelectViewState` / `ResultViewState` / `DecideChart` answering property ids, the private 20001–20315 ids, `FrameExtra` row and series state), and the object modules `notes`, `gauge`, `judge`, `songlist`, `covers`, `graphs` (gauge / judge / bpm / timing distribution / timing visualizer / hit error / density), `color` (`parse_hex_color`). `Skin::with_document_lanes` rebuilds the lane geometry from a document's `note` destinations.
...
- **Key invariants & algorithms**:
  ...
  - **Skin lane layout**: `dual` DP (two fields, P1 left/P2 right, turntables on the *outer* edge, ignoring SP `scratch_left`) only when `dual_field && players>=2 && key%players==0`; the right field is sized to clear the BGA column (`right_limit = bga.x − 24`); a BGA whose right edge lies within `DUAL_LEFT_BGA_LIMIT` of the screen width counts as sitting *left* of the fields and the two fields start after it (`DUAL_MARGIN`) instead of collapsing beside it (v3 dual documents). SP uses `scratch_left` to order scratch first/last. `lift` clamps to `0.0..=0.9` and raises `judge_y` toward `top_y`; `beam_height_frac` clamps to `0..=1`. `lane_bg`/`outline`/`divider` keep their configured alpha (not forced opaque). `bomb_us = bomb_duration_ms * 1000`.
...
- **Public API**:
  ...
  - `SkinOptions` (`Config.skin`): `folder`, `screen`, `selected` (document per `SkinType` id), `custom` (per-document `SkinCustomisation` keyed by path), `shared` (per-bundle customisation keyed by `bundle_key`), `default_skin_installed`; `customisation` / `customise`, `shared_customisation` / `shared_customise`, `move_shared` (generation migration), `user_config(path)` merges the bundle's shared choices under the document's own.
...
Reference-compatible JSON skin support (Phase E, 2026-09-09~10). Modules: `timer` (the 151 `TIMER_*` ids, generated by `tools/gen-skin-timer.rs`), `dst` (destination keyframe interpolation with the reference `prepareRegion` semantics and op/draw gating), `property` (the 968 property ids generated by `tools/gen-skin-property.rs` plus the `SkinStateSource` mapping to player state), `model` (serde mirror of the JSON skin document), `loader` (json5 parsing with size limits, wildcard/random file selection, filemap, customfile, stretch and dstfilter rules, user option/file/offset settings), `lua` (sandboxed expression evaluation through `mlua` with instruction and memory budgets; no io/os/require, text chunks only), `resolve` (skin-root-only path resolution, traversal rejected). Errors are `SkinError` (thiserror). Features `json5` and `lua` are optional and enabled by the player. Rendering of a loaded skin lives in `rbms-render::skin_render`; the SKIN settings tab and per-screen selection live in `apps/rbms-player/src/skin_select*` with persistence through `rbms-config` (`Config.skin`). Generated tables carry a checksum test that re-derives them from the reference constants.

2026-09-17 additions: the loader assembles nested destinations into `LoadedSkin.nested` (`NestedTracks`: note group / bpm / stop / time tracks, `JudgeTracks` images and numbers, `SongListTracks` nine arrays plus graph) so the renderer never compiles a track twice; the model carries `SkinDef.replace` (`LoadedSkin::replace_names()`), `hotspot` (`HotspotDef { id, action }`, validated against `HOTSPOT_ACTIONS`: folders, modal-close, modal-replay, records, search, settings, sort, tables), `scope: 'bundle'` on `property` / `filepath` / `offset` headers (`CustomFile.scope`), and the rbms-only `densitygraph` object. Tests: `tests/skin_nested.rs` over `tests/fixtures/nested/`.
