...
> **현재 합의(2026-09-16):** 저장소는 dev-only로 운용합니다. 이 문서의 옵션·명령은 Phase R에서만 다시 검토하며, `prod` 생성·보호·태그·서명·공개 배포는 사용자가 실제 키를 제공한 뒤 함께 수행합니다. 에이전트 단독으로 릴리스를 시작하지 않습니다. 현재 작업 정본은 [roadmap](../roadmap.md)입니다.
...
**Phase R 대기** — 사용자 키와 최종 배포 결정을 받은 뒤 위 당시 옵션을 실제 원격·워크플로 상태와 재검증한다. 현재 dev-only이며, 이 문서의 과거 원격 SHA·CI 결과는 당시 관찰값이다.
