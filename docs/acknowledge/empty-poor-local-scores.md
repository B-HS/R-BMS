...
## 1. 空POOR(empty poor) 동작 — beatoraja 7K 준거
...
- beatoraja는 `score.addJudgeCount(5, …)`로 **MISS 카운트**를 증가시킨다. 우리는 별도 `empty_poor`
...
- 게이지 페널티는 `Judge::Miss` 델타(MS) 사용 — beatoraja judge 5와 동일 인덱스. 기존(노트 소비 +
...
- 램프는 beatoraja `ClearType.id`(0..10) 정수로 영속 → 색/라벨 왕복(`clear_type_from_id`). LightAssistEasy(3)
