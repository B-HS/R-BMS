...
차트의 `#BMPxx` 가 그림 대신 동영상 파일을 가리키면 그 배경은 동영상으로 재생된다. 이름 해석은 레퍼런스 구현과 같은 확장자 목록(`.mpg .mpeg .m1v .m2v .mp4 .m4v .avi .wmv .webm`)을 쓰고, 같은 `#BMPxx` 목록을 그림용과 동영상용으로 두 번 훑어 서로가 가져가지 않은 것만 남긴다. 그려지는 자리는 그림과 완전히 같다 — 문서의 `bga` 객체, 또는 문서가 배경을 선언하지 않았을 때의 내장 사각형이다. `DISPLAY` 의 BGA 를 끄면 파일을 열지도 않는다.

**재생되는 컨테이너**는 순수 Rust 디코더가 있는 두 가지다.

| 확장자 | 코덱 | 비고 |
|---|---|---|
| `.mpg` `.mpeg` `.m1v` `.m2v` | MPEG-1 / MPEG-2 비디오 | 프로그램 스트림과 엘리멘터리 스트림 모두. ISO/IEC 11172-1 시스템 스트림(대부분의 `.mpg` BGA)도 읽는다 |
| `.mp4` `.m4v` | H.264 | 베이스라인 스트림 기준 |

`.avi` `.wmv` `.webm` 은 확장자만 인식한다. 이 셋을 그림으로 잘못 읽지 않고 건너뛰되, 순수 Rust 디코더가 없으므로 그 id 의 배경은 비어 있고 콘솔에 한 줄 경고가 남는다.

**동기화 규칙**은 레퍼런스 구현과 같다. 탐색(seek)이 없다.

- BGA 타임라인이 그 id 로 넘어간 곡 시간이 곧 0번 프레임의 시간이다(그 시간이 오프셋이 된다).
- 매 프레임 그 시점 이하의 가장 최신 프레임을 고르고, 지나간 프레임은 버린다. 디코더가 뒤처져도 곡이 기다리지 않는다.
- 스트림이 끝나면 배경은 비워지고 그대로 남는다. 반복하지 않고 마지막 프레임을 붙들지도 않는다.
- 이미 시작한 배경으로 타임라인이 되돌아와도 되감지 않는다. 되감을 탐색이 없기 때문이다.

**한계**

- MPEG-1/2 디코더에 스트리밍 진입점이 없어 워커 스레드가 한 시퀀스를 통째로 디코드한다. 차트가 싣는 짧은 루프에는 맞지만 긴 영상에는 메모리가 크다.
- 색 변환은 BT.601 스튜디오 스윙 고정이고 크로마는 최근접 확대다.
- 동영상마다 워커 스레드가 하나씩 생긴다. 한 차트가 동영상을 여럿 선언하면 그만큼 열린다.
