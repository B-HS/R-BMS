...
const ROW_FOCUS_H: f32 = 44.0;
const ROW_GAP: f32 = 4.0;

const TITLE_FOCUS: Color = Color::rgb(244, 236, 156);
const TITLE_DIM: Color = Color::rgb(190, 186, 150);
const FOCUS_BORDER: Color = Color::rgb(120, 200, 240);
const PANEL: Color = Color::rgb(14, 16, 26);
const PANEL_HI: Color = Color::rgb(22, 24, 36);
const DIVIDER: Color = Color::rgb(40, 42, 58);
const CYAN: Color = Color::rgb(120, 205, 235);
...
pub fn render_select<R: Renderer>(r: &mut R, v: &SelectView) -> Vec<(Rect, SelectHot)> {
    let mut hot = Vec::new();
    r.clear(Color::rgb(8, 8, 14));
    r.fill_rect(Rect::new(0.0, 0.0, 1280.0, 52.0), Color::rgb(18, 18, 30));
    draw_text(r, LIST_X, 12.0, 2.6, Color::WHITE, "MUSIC SELECT");
    if !v.header.is_empty() {
        draw_text(r, 300.0, 22.0, 1.3, Color::GRAY, &fit_text(&v.header, 1.3, LIST_X + LIST_W - 300.0 - 80.0));
    }
    let m = v.rows.len();
    draw_text_right(r, LIST_X + LIST_W, 22.0, 1.5, Color::GRAY, &format!("{}/{}", (v.sel + 1).min(m.max(1)), m));
    draw_text(r, LIST_X, BOTTOM + 14.0, 1.1, Color::GRAY, v.guide);
...
fn render_list<R: Renderer>(r: &mut R, v: &SelectView, hot: &mut Vec<(Rect, SelectHot)>) {
    let m = v.rows.len();
    if m == 0 {
        draw_text(r, LIST_X + 12.0, TOP + 40.0, 1.5, Color::GRAY, "NO CHARTS");
        return;
    }
    ...
    for idx in start..(start + visible).min(m) {
        ...
        let h = if focused { ROW_FOCUS_H } else { ROW_H };
        let bg = if focused {
            Color::rgb(50, 56, 82)
        } else if row.folder {
            Color::rgb(40, 44, 30)
        ...
        if focused {
            outline(r, Rect::new(LIST_X, y, LIST_W, h), 2.0, FOCUS_BORDER);
        }
        ...
        if row.folder {
            ...
            if let Some(n) = row.folder_count {
                draw_text_right(r, LIST_X + LIST_W - 20.0, cy - 8.0, 1.3, Color::GRAY, &format!("{n} CHARTS"));
            }
        } else {
            ...
            let title_color = if focused { TITLE_FOCUS } else { TITLE_DIM };
...
fn render_song_detail<R: Renderer>(r: &mut R, v: &SelectView, d: &DetailView, hot: &mut Vec<(Rect, SelectHot)>) {
    ...
    outline(r, Rect::new(DETAIL_X, TOP, DETAIL_W, BOTTOM - TOP), 1.0, DIVIDER);
    ...
    match d.cover {
        ...
        CoverState::Loading => {
            ...
            draw_text_centered(r, cr.x + cr.w * 0.5, cr.y + cr.h * 0.5 - 7.0, 1.2, Color::GRAY, "LOADING");
        }
        CoverState::None => {
            r.fill_rect(cr, Color::rgb(16, 18, 28));
    ...
    let mut ty = 86.0;
    draw_text(r, tx, ty, 2.0, Color::WHITE, &l1);
    ty += 30.0;
    if let Some(l2) = &l2 {
        draw_text(r, tx, ty, 2.0, Color::WHITE, l2);
        ty += 30.0;
    } else if !d.subtitle.is_empty() {
        draw_text(r, tx, ty, 1.2, Color::rgb(160, 160, 178), &fit_text(&d.subtitle, 1.2, tw));
        ty += 24.0;
    }
    if !d.artist.is_empty() {
        draw_text(r, tx, ty, 1.3, CYAN, &fit_text(&d.artist, 1.3, tw));
        ty += 24.0;
    }
    if !d.genre_maker.is_empty() {
        draw_text(r, tx, ty, 1.1, Color::GRAY, &fit_text(&d.genre_maker, 1.1, tw));
    }
    ...
    r.fill_rect(Rect::new(inner_x, 254.0, inner_w, 2.0), DIVIDER);
    ...
    for (i, cell) in d.stats.iter().take(6).enumerate() {
        ...
        draw_text(r, cx, cy, 1.0, Color::GRAY, cell.label);
        draw_text(r, cx, cy + 14.0, 1.4, Color::WHITE, &fit_text(&cell.value, 1.4, col_w - 12.0));
    }

    // Note-density histogram.
    r.fill_rect(Rect::new(inner_x, 348.0, inner_w, 2.0), DIVIDER);
    draw_text(r, inner_x, 362.0, 1.3, Color::rgb(200, 205, 220), "DENSITY");
    let box_y = 380.0;
    let box_h = 74.0;
    ...
    match &d.density {
        ...
        _ => draw_text_centered(r, inner_x + inner_w * 0.5, box_y + box_h * 0.5 - 7.0, 1.2, Color::GRAY, "…"),
    }
    outline(r, box_rect, 1.0, DIVIDER);
...
fn render_records<R: Renderer>(r: &mut R, v: &SelectView, rec: &RecordsView, hot: &mut Vec<(Rect, SelectHot)>) {
    let inner_x = DETAIL_X + 16.0;
    let inner_w = DETAIL_W - 32.0;
    draw_text(r, inner_x, 468.0, 1.4, Color::WHITE, "RECORDS");
    draw_text_right(r, DETAIL_X + DETAIL_W - 16.0, 472.0, 1.2, Color::GRAY, &format!("{} CLEAR / {} PLAYS", rec.clears, rec.plays));
    let Some(best) = &rec.best else {
        draw_text(r, inner_x, 508.0, 1.3, Color::GRAY, "NO PLAY YET");
        return;
    };
    ...
    for (ri, rec_row) in rec.recent.iter().take(max_rows).enumerate() {
        ...
        r.fill_rect(Rect::new(inner_x, y, inner_w, rh - 4.0), PANEL);
        r.fill_rect(Rect::new(inner_x, y, 6.0, rh - 4.0), rec_row.lamp);
        draw_text(r, inner_x + 14.0, y + 3.0, 1.1, Color::rgb(180, 180, 195), &rec_row.when);
        ...
        draw_text_right(r, inner_x + inner_w - 12.0, y + 3.0, 1.2, Color::WHITE, &ex_txt);
        match &rec_row.trend {
            Some((t, c)) => {
                draw_text_right(r, inner_x + inner_w - 80.0, y + 17.0, 1.0, *c, t);
                draw_text_right(r, inner_x + inner_w - 12.0, y + 17.0, 1.0, Color::GRAY, &format!("BP {}", rec_row.bp));
            }
            None => draw_text_right(r, inner_x + inner_w - 12.0, y + 17.0, 1.0, Color::GRAY, &format!("BP {}", rec_row.bp)),
        }
        if !modal_open {
...
fn render_folder_detail<R: Renderer>(r: &mut R, label: &str, count: usize) {
    ...
    r.fill_rect(Rect::new(DETAIL_X + 16.0, 96.0, DETAIL_W - 32.0, 120.0), PANEL_HI);
    draw_text(r, DETAIL_X + 40.0, 120.0, 2.4, Color::WHITE, "FOLDER");
    draw_text(r, DETAIL_X + 40.0, 168.0, 1.6, Color::rgb(210, 210, 150), &fit_text(label, 1.6, DETAIL_W - 80.0));
    if count > 0 {
        draw_text(r, DETAIL_X + 40.0, 240.0, 1.4, Color::GRAY, &format!("{count} CHARTS"));
    }
}
...
fn render_modal<R: Renderer>(r: &mut R, modal: &SelectModal, hot: &mut Vec<(Rect, SelectHot)>) {
    ...
    let my = (720.0 - mh) * 0.5;
    r.fill_rect(Rect::new(mx, my, mw, mh), Color::rgb(20, 22, 32));
    ...
    draw_text(r, mx + 20.0, my + 72.0, 1.5, Color::rgb(210, 210, 225), &fit_text(&modal.title, 1.5, mw - 40.0));
    draw_text(r, mx + 20.0, my + 100.0, 1.3, Color::GRAY, &modal.sub);
    ...
    for k in 0..6 {
        ...
        draw_text_right(r, mx + 330.0, y, 1.7, Color::WHITE, &modal.counts[k].to_string());
    }
    let rx = mx + 380.0;
    draw_text(r, rx, my + 138.0, 1.7, Color::WHITE, &format!("EX  {} / {}", modal.ex, modal.max_ex));
    ...
    draw_text(r, rx, my + 228.0, 1.7, Color::GRAY, &format!("EMPTY POOR  {}", modal.empty_poor));
    draw_text(r, rx, my + 258.0, 1.7, modal.clear_color, &format!("GAUGE  {}%", modal.gauge_value));
    draw_text(r, rx, my + 288.0, 1.3, Color::GRAY, &format!("{} / {}", modal.index + 1, modal.total));

    let by = my + mh - 52.0;
    if modal.has_replay {
        let rect = Rect::new(mx + 20.0, by, 220.0, 36.0);
        r.fill_rect(rect, Color::rgb(70, 120, 80));
        draw_text(r, mx + 36.0, by + 9.0, 1.6, Color::WHITE, "PLAY REPLAY");
        hot.push((rect, SelectHot::ModalReplay));
    } else {
        draw_text(r, mx + 24.0, by + 9.0, 1.3, Color::GRAY, "NO REPLAY SAVED");
    }
    draw_text(r, mx + 20.0, my + mh - 92.0, 1.1, Color::GRAY, "UP DOWN PREV/NEXT   ENTER REPLAY   ESC CLOSE");
    let close = Rect::new(mx + mw - 120.0, by, 100.0, 36.0);
    r.fill_rect(close, Color::rgb(90, 70, 80));
    draw_text(r, mx + mw - 96.0, by + 9.0, 1.6, Color::WHITE, "CLOSE");
