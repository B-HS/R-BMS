...
fn digit_out_of_range_returns_none() {
    assert_eq!(digit(b'9', 8), None);
    assert_eq!(digit(b'A', 10), None);
    assert_eq!(digit(b'Z', 16), None);
    assert_eq!(digit(b'G', 16), None);
...
fn non_ascii_char_in_slot_skipped() {
    ...
    bytes.extend_from_slice("あ05".as_bytes());
    bytes.extend_from_slice(b"\r\n");
    let s = parse(&bytes);
    assert!(s.measures.get(&1).unwrap().channels.is_empty());
}
...
fn channel_03_hex_pairs_value16() {
    ...
    assert_eq!(ch.len(), 3);
    assert_eq!(ch[0].value16(), 10);
    assert_eq!(ch[1].value16(), 16);
    assert_eq!(ch[2].value16(), 255);
}
...
fn bpm_def_invalid_value_not_inserted() {
    ...
    assert!(!s.bpm_def.contains_key(&1));
}
...
fn random_exactly_one_branch_selected() {
    ...
    for seed in [0u64, 1, 2, 7, 42, 100, 1234, u64::MAX] {
        ...
        let count = (1..=4u32).filter(|id| s.wav.contains_key(id)).count();
